...
public class StreamLabService {
    ...
    public List<Product> RProblemThree()
    {
        ...
        // Return the list
    	return products.findAll().stream().filter(c -> c.getName().contains("s")).toList();
    }
    ...
    public List<User> RProblemFour()
    {
        ...
        GregorianCalendar ex = new GregorianCalendar(2016, Calendar.JANUARY, 01);
    	Date date1 = ex.getTime();
        return users.findAll().stream().filter((a)-> a.getRegistrationDate().before(date1)).toList();
    }
    ...
    public List<User> RProblemFive()
    {
        ...
        // Return the list
        GregorianCalendar startDate = new GregorianCalendar(2016, Calendar.DECEMBER, 31);
        Date date1 = startDate.getTime();
        GregorianCalendar  endDate= new GregorianCalendar(2018, Calendar.JANUARY, 01);
        Date date2 = endDate.getTime();
        return users.findAll().stream().filter((a)-> a.getRegistrationDate().before(date2)&& a.getRegistrationDate().after(date1)).toList();
    }
    ...
    public List<ShoppingcartItem> RProblemSix()
    {
        // Write a query that retrieves all of the products in the shopping cart of the user who has the email "afton@gmail.com".
        // Return the list
        User example1 = users.findAll().stream().filter(a-> a.getEmail().equals("afton@gmail.com")).findFirst().orElse(null);
       List<ShoppingcartItem> products12 = shoppingcartitems.findAll().stream().filter(a -> a.getUser().equals(example1)).toList();

//        List<Product> products21 = products.findAll().stream().filter(z->z.getId().equals(getProduct(products12.))).toList();
    	return products12;
    }
